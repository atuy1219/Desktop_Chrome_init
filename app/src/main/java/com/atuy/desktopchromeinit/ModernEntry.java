 ...
 *
 * Chrome Desktop keeps browser code in split APKs, so hook installation must
 * not depend on a single lifecycle callback. The module retries until the
 * ChromeTabbedActivity a3 crash guard has actually returned a HookHandle.
 */
public final class ModernEntry extends XposedModule {
    ...
    private static final int MAX_RETRY_COUNT = 240; // 60 seconds
    ...
    private synchronized void tryInstall(String phase) {
        ...
        if (loader == null) {
            d(phase + ": no usable Chrome ClassLoader yet");
            scheduleRetry(RETRY_INTERVAL_MS);
            return;
        }
        ...
        try {
            d(phase + ": attempting hook install with loader=" + loader);
            new ChromeInitHook().handleLoadPackage(compat);

            if (ChromeInitHook.isExtensionsMenuGuardInstalled()) {
                installed = true;
                latestClassLoader = loader;
                d(phase + ": a3 crash guard confirmed installed");
            } else {
                d(phase + ": a3 crash guard not installed; retrying");
                scheduleRetry(RETRY_INTERVAL_MS);
            }
        } catch (Throwable t) {
    ...
    private ClassLoader resolveUsableClassLoader() {
        ...
        loader = currentApplicationClassLoader();
        if (canLoadChromeActivity(loader)) {
            latestClassLoader = loader;
            return loader;
        }
    ...
    private static boolean canLoadChromeActivity(ClassLoader loader) {
        ...
        try {
            Class.forName(
                    ...
                    false,
                    loader);
            Class.forName("u1h", false, loader);
            return true;
    ...
    private static ClassLoader currentApplicationClassLoader() {
        try {
            ...
            if (app instanceof Application) {
                return ((Application) app).getClassLoader();
            }
        } catch (Throwable ignored) {
        }
        return null;
    ...
    private void d(String message) {
        // Always emit ordinary logcat in addition to the framework module log.
        Log.i(TAG, message);
        try {
